...
For commands that don't need to execute anything else in order to work, you can use libcallfilt's C<denyexec> filter to block any calls they make to the libc functions for running external commands. That way if you make a mistake in configuring the option restritcions, you have a second line of defense.
